...
def test_load_config_defaults(mock_args):
    ...
    with patch.dict(os.environ, {}, clear=True):
        ...
        assert config["spoolman_url"] == "http://localhost:7912/api"
...
async def test_spool2klipper_init():
    ...
    assert agent.spoolman_url == "http://localhost:7912/api"
...
async def test_moonraker_send_gcode_success(mock_moonraker_data):
    ...
    mock_script.assert_called_with(script="SET_GCODE_VARIABLE MACRO=test", _notification=True)
